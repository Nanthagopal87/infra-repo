...
# CONFIG
# -------------------------
PROJECT_ID="your-project-id" # <--- REMEMBER TO UPDATE THIS
REGION="asia-south1"
ZONE="${REGION}-a"
...
PROD_FW_PSC_INGRESS="prod-fw-allow-psc-ingress" # New Firewall Rule
...
# This is usually for egress to the internet, not directly for PSC traffic
...
# NEW: Allow incoming HTTP traffic from the PSC NAT Subnet to NGINX VMs
gcloud compute firewall-rules create $PROD_FW_PSC_INGRESS \
  --project=$PROJECT_ID --network=$PROD_VPC \
  --allow=tcp:80 \
...
# Allow all egress for producer VMs (e.g., for apt updates, NAT to internet)
...
# -------------------------
echo "Creating Regional TCP Health Check..."
# Health check must be regional and match backend service protocol
gcloud compute health-checks create tcp $HC_NAME \
  --project=$PROJECT_ID --region=$REGION --port=80

# -------------------------
echo "Creating Regional Backend Service..."
# Must use region-backend-services for internal TCP/UDP load balancer for PSC
gcloud compute region-backend-services create $BS_NAME \
  --project=$PROJECT_ID --region=$REGION \
  --protocol=TCP \
  --health-checks=$HC_NAME \
  --load-balancing-scheme=INTERNAL # Specify INTERNAL for classic NetLB

gcloud compute region-backend-services add-backend $BS_NAME \
  --project=$PROJECT_ID --region=$REGION \
...
echo "Testing connection from consumer VM. This might take a moment for full propagation..."
# Give it a small delay for forwarding rules and backend health to propagate
sleep 30
gcloud compute ssh $CLIENT_VM --project=$PROJECT_ID --zone=$ZONE --tunnel-through-iap --command "
curl -v http://$EP_IP
"

echo "✅ PSC setup completed successfully (pending curl output)."
